...
function initializeHeroCanvas() {
    ...
    // Configuration
    const blobCount = 6;
    // Editorial Palette: Peru, Soft Blue, Beige, Lavender, Off-White
    const colors = ['rgba(205, 133, 63, 0.6)', 'rgba(224, 192, 151, 0.6)', 'rgba(176, 196, 222, 0.6)', 'rgba(216, 191, 216, 0.5)', 'rgba(245, 245, 245, 0.8)'];
    ...
    class Blob {
        ...
        init() {
            ...
            this.radius = Math.random() * 150 + 100; // Large orbs
        ...
        update() {
            ...
            // Friction to return to slow speed
    ...
    function animate() {
        ...
        ctx.filter = 'blur(50px)'; // Critical for the fluid look
        blobs.forEach(blob => {
            blob.update();
            blob.draw();
        });
...
// === Fluid Hero Canvas END ===

// === 3D Tilt Effect START ===
function initializeTiltEffect() {
    const cards = document.querySelectorAll('.project-showcase-item');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeOtherFeaturesWithoutAudio() {
        ...
        initializeTiltEffect(); // Added 3D Tilt
        initializeCountdown();
